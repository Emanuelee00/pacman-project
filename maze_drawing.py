...
CORNER_DRAWER = {
    1: lambda surface, x_start, y_start: _draw_arc_twice(
        surface, x_start + WS // 2, y_start, (WS // 2) - 1, 0, 180, COLOR
        ),
    2: lambda surface, x_start, y_start: _draw_arc_twice(
        surface, x_start + WS, y_start + WS // 2, (WS // 2) - 1, 90, 270, COLOR
        ),
    3: lambda surface, x_start, y_start: _draw_arc_twice(
        surface, x_start + WS, y_start, WS, 90, 180, COLOR
        ),
    4: lambda surface, x_start, y_start: _draw_arc_twice(
        surface, x_start + WS // 2, y_start + WS, (WS // 2) - 1, 180, 360, COLOR
        ),
    5: lambda surface, x_start, y_start: (
        pygame.draw.line(surface, COLOR, (x_start, y_start), (x_start, y_start + WS), width=2),
        pygame.draw.line(surface, COLOR, (x_start + WS, y_start), (x_start + WS, y_start + WS), width=2)
        ),
    6: lambda surface, x_start, y_start: _draw_arc_twice(
        surface, x_start + WS, y_start + WS, WS, 180, 270, COLOR
        ),
    7: lambda surface, x_start, y_start: pygame.draw.line(
        surface, COLOR, (x_start, y_start), (x_start, y_start + WS), width=2
        ),
    8: lambda surface, x_start, y_start: _draw_arc_twice(
        surface, x_start, y_start + WS // 2, (WS // 2) - 1, 270, 450, COLOR
        ),
    9: lambda surface, x_start, y_start: _draw_arc_twice(
        surface, x_start, y_start, WS, 0, 90, COLOR
        ),
    10: lambda surface, x_start, y_start: (
        pygame.draw.line(surface, COLOR, (x_start, y_start), (x_start + WS, y_start), width=2),
        pygame.draw.line(surface, COLOR, (x_start, y_start + WS), (x_start + WS, y_start + WS), width=2)
        ),
    11: lambda surface, x_start, y_start: pygame.draw.line(
        surface, COLOR, (x_start, y_start + WS), (x_start + WS, y_start + WS), width=2
        ),
    12: lambda surface, x_start, y_start: _draw_arc_twice(
        surface, x_start, y_start + WS, WS, 270, 360, COLOR
        ),
    13: lambda surface, x_start, y_start: pygame.draw.line(
        surface, COLOR, (x_start + WS, y_start), (x_start + WS, y_start + WS), width=2
        ),
    14: lambda surface, x_start, y_start: pygame.draw.line(
        surface, COLOR, (x_start, y_start), (x_start + WS, y_start), width=2
        ),
}
...
def draw_maze(surface, tile_map):
    ...
    for ty in range(2 * HEIGHT + 1):
        for tx in range(2 * WIDTH + 1):
            ...
            if tile_map[ty][tx] == Tile.WALL:
                if ty % 2 == 0:
                    pygame.draw.line(surface, "blue", (x_start, y_start), (x_start + FLOOR_SIZE, y_start), width=2)
                    pygame.draw.line(surface, "blue", (x_start, y_start + WALL_SIZE), (x_start + FLOOR_SIZE, y_start + WALL_SIZE), width=2)
                if tx % 2 == 0:
                    pygame.draw.line(surface, "blue", (x_start, y_start), (x_start, y_start + FLOOR_SIZE), width=2)
                    pygame.draw.line(surface, "blue", (x_start + WALL_SIZE, y_start), (x_start + WALL_SIZE, y_start + FLOOR_SIZE), width=2)
...
if __name__ == "__main__":
    ...
    draw_maze(maze_surface, tile_map)
